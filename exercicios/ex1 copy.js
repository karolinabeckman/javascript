...
function entregaProsPais(aluno) {
	...
	const resultadoFinal = situacaoFinal(situacoesAluno)

  return resultadoFinal
}

function situacaoFinal(situacoes){
  let qtdeMateriasRecuperacao = 0
  for (let situacao of situacoes){
		if(situacao === 'Reprovado') return 'Reprovado' 
		if (situacao === 'Recuperação'){
			qtdeMateriasRecuperacao++
			if(qtdeMateriasRecuperacao > 2) return 'Reprovado'
		}
	}
	return qtdeMateriasRecuperacao === 0 ? 'Aprovado' : 'Recuperação'
}
